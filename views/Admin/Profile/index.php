...
<main class="content">
	...
	<button id="launch-modal" type="button" class="btn btn-primary d-none" data-bs-toggle="modal" data-bs-target="#ModalProfileImage">
		Launch modal
	</button>
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				...
				<div class="modal-body">
					Click save to change profile image !
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary" data-bs-dismiss="modal" onclick="cleanInput()" id="modal-close">Close</button>
			...
			<div class="col-md-4 col-xl-3">
				<div class="card mb-3">
					...
					<div class="card-body text-center">
						<div class="position-relative rounded-circle mx-auto mb-3 img-overlay bg-secondary" style="width:128px; height: 128px;">
							<img src="<?=$config->get('app.url')?>blog/assets/images/avatars/<?= $user->profile_image ?? "default.png" ?>" alt="Christina Mason" class="img-fluid rounded-circle mb-2" width="128" height="128" />
							<div class="overlay d-none">
								<div class="text-white w-100 h-100">
									<form class="flex-grow-1 h-100" action="/blog/admin/profile/image/save" method="POST" id="change-profile-image" enctype="multipart/form-data">
										<label class="h-100 d-flex justify-content-center align-items-center" for="profile-image"><i class="fa-regular fa-images fs-1"></i></label>
			...
			<form action="/blog/admin/profile/save" method="POST" id="profile">
				<fieldset>
					...
					<div class="mb-3">
						...
						<input class="form-control form-control-lg" type="text" name="first_name" value="<?= $request->first_name ?? $user->first_name ?>" required/>
						<small class="d-block invalid-feedback fs-5"></small>
					</div>

					<div class="mb-3">
						<label class="form-label">Last Name</label>
						<input class="form-control form-control-lg" type="text" name="last_name" value="<?= $request->last_name ?? $user->last_name?>" required/>
						<small class="d-block invalid-feedback fs-5"></small>
					</div>

					<div class="mb-3">
						<label class="form-label">Phone</label>
						<input class="form-control form-control-lg" type="text" name="phone" value="<?= $request->phone ?? $user->phone?>" required/>
						<small class="d-block invalid-feedback fs-5"></small>
					</div>

					<div class="mb-3">
						<label class="form-label">Email</label>
						<input class="form-control form-control-lg" type="email" name="email" value="<?= $request->email ?? $user->email?>" required/>
						<small class="d-block invalid-feedback fs-5"></small>
					</div>

					<div class="mb-3">
						<label class="form-label">Username</label>
						<input class="form-control form-control-lg" type="text" name="username" value="<?= $request->username ?? $user->username?>" required/>
						<small class="d-block invalid-feedback fs-5"></small>
					</div>
...
<script>
	...
	function setModal(modal, type, title, body, enableFooter = true) {
		...
		enableFooter ? modal.querySelector('.modal-footer').classList.remove('d-none') : modal.querySelector('.modal-footer').classList.add('d-none');
	}
